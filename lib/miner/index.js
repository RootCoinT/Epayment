...
class Miner {
    ...
    mine(rewardAddress, feeAddress) {
        ...
        // 添加开始挖矿的日志
        console.info('\n=== 开始挖矿 ===');
        console.info(`区块高度: ${baseBlock.index}`);
        console.info(`当前难度值: ${difficulty}`);
        ...
        const promise = thread.promise().then((result) => {
            ...
            console.info('\n=== 挖矿完成 ===');
            console.info(`耗时: ${(mineEndTime - mineStartTime)/1000} 秒`);
    ...
    static proveWorkFor(jsonBlock, targetDifficulty) {
        ...
        do {
            ...
            if (hashCount % 1000000 === 0) {
                ...
                console.info(`当前难度(16进制): 0x${currentDifficulty.toString(16).padStart(14, '0')}`);
                console.info(`目标难度(16进制): 0x${targetDifficulty.toString(16).padStart(14, '0')}`);
                console.info(`已用时间11: ${elapsedTime}ms`);
            }
        ...
        console.info(`\n挖矿完成:`);
        console.info(`生成时间: ${block.generationTime}ms`);
        console.info(`当前哈希: 0x${block.hash.substring(0, 14)}`);
        console.info(`计算次数: ${hashCount}`);
        console.info(`目标难度: 0x${targetDifficulty.toString(16).padStart(14, '0')}`);
        console.info(`平均哈希率: ${Math.floor(hashCount / (block.generationTime / 1000))} H/s`);
